...
// netlify/functions/recentTracks copy.ts
var recentTracks_copy_exports = {};
__export(recentTracks_copy_exports, {
  handler: () => handler
});

// netlify/functions/api/lastFm/getLastFMResponse.ts
var import_axios = __toESM(require_axios2());
require_main().config({ path: "./.env.local" });
var lastFMUrl = "http://ws.audioscrobbler.com/2.0/";
async function getLastFMResponse({
  params,
  event,
  responseToItems: responseToItems2,
  responseToMeta: responseToMeta2
}) {
  var _a, _b;
  try {
    ...
    const response = await import_axios.default.get(lastFMUrl, {
      params: __spreadProps(__spreadValues(__spreadValues({}, event.queryStringParameters), params), {
        user: LASTFM_USER,
        api_key: LASTFM_API_KEY,
        format: "json"
      })
    });
    const items = responseToItems2(response);
    const meta = responseToMeta2(response);
    return {
      headers: {
        "Content-Type": "application/json"
      },
      statusCode: response.status,
      body: JSON.stringify(__spreadProps(__spreadValues({}, items), { meta }))
    };
  } catch (error) {
    const errorMessage = ((_a = error == null ? void 0 : error.response) == null ? void 0 : _a.data) ?? error.toString() ?? {};
...
// netlify/functions/api/lastFm/helpers.ts
function handleLastFMImages(images) {
  const flatImages = {};
...
function responseToItems(response) {
  const recentTracks = [];
  response.data.recenttracks.track.forEach((_track, index) => {
    const track = {
      id: _track.mbid,
      name: _track.name,
      album: {
      ...
      date: new Date(Number(_track.date.uts * 1e3)).toISOString(),
      url: _track.url,
...
module.exports = __toCommonJS(recentTracks_copy_exports);
 ...
 * MIT Licensed
 */
//# sourceMappingURL=recentTracks copy.js.map
